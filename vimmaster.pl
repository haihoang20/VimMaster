...
% eg: attribute(Topic) means that Topic is associated with attribute, and attribute should therefore be a keyword that points to Topic.
...
solution(generalhelp, "Welcome to Vimmaster! ask your vim questions here with the 'ask' predicate, like ask('how do you move?'), or ask('can you jump to the end of the file in vim?'). Or, if you want to learn about automatically generating a vimrc file, try vimrc_help().").
...
solution(howtoinserttostartofline, "To insert at the beginning of the line, type 'I'.").
solution(howtoinserttoendofline, "To insert to the end of the line, type 'A'.").
...
% if these words appear, they will be ignored.
...
% remove_char(S, C, X): removes character C from string S, and puts the result in X.
remove_char(S,C,X) :- atom_concat(L,R,S), atom_concat(C,W,R), atom_concat(L,W,X).
remove_char(S,_,S).

% ask_helper(Q, T): T is the answer to the question given by string Q.
% example queries:
% ask_helper("how do you move",X).
ask_helper(Q,T) :-
    remove_char(Q, '?', Q2),
    string_lower(Q2, Q3),
...
% ask(Q): prints the answer to question Q
% example query:
% ask("how do you quit?").
ask(Q):-
  ask_helper(Q, T),
...
% vim_help() prints out a message that gives general help advice about how to use vimmaster.
vim_help() :-
    ask('help').
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
vimrc_help() :-
    print('To generate a vimrc file, call the predicate create_vimrc, passing in a list of the features you want to include. Here is a list of all the features that you can choose from:'),
    nl(),
    nl(),
    getAllFeatures(L),
    print(L),
